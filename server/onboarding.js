...
const { hasKey, structureCorpus, processFile, MAX_STORED_TEXT_CHARS } = require("./ingest");
...
// claude/structureCorpus/processFile now live in ./ingest, shared with the
// Teach Your Brain surface (server.js's /api/teach/upload).
...
async function runBrainDump(token, { text, files }) {
  ...
  const processed = [];
  for (const f of incoming) processed.push(await processFile(f, "onboarding", onboarding.id));
  ...
  const extractedProfile = await structureCorpus(corpus);
  // Images return their own structured facts directly (a photo of a menu
  // shouldn't depend on structureCorpus re-parsing a plain-text summary
  // of itself to surface its facts) — folded in alongside whatever
  // structureCorpus found in the combined text corpus.
  ...
  onboarding.data.brainDump = {
    ...
    // still include earlier batches' extracted text; structureCorpus
